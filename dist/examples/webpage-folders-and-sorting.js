(() => {
  // examples/webpage-folders-and-sorting.js
  AMF.plugin("Webpage Folders & Sorting", () => {
    AMF.on("content-frame-load", ({ contentWindow }) => {
      ...
      function saveState() {
        ...
        sessionStorage.setItem(FOLDER_KEY, JSON.stringify(Array.from(expandedFolders)));
      }
      function loadState() {
        try {
      ...
      function buildDataTree(trs2) {
        ...
        trs2.forEach((tr) => {
          ...
          for (let i = 0;i < parts.length; i++) {
            ...
            let child = node.children.find((c) => c.name === part && c.type === (i === parts.length - 1 ? "file" : "folder"));
            if (!child) {
              child = {
                name: part,
                path: currentPath,
      ...
      function nodeMatchesFilter(node, filter) {
        ...
        return node.children && node.children.some((child) => nodeMatchesFilter(child, filter));
      }
      function sortTree(node) {
        ...
        node.children.sort((a, b) => {
          ...
          if (currentSort === "page") {
            ...
            aText = a.type === "file" && a.row ? new Date(a.row.querySelector("td:nth-child(3)").textContent.trim()) : new Date(0);
            bText = b.type === "file" && b.row ? new Date(b.row.querySelector("td:nth-child(3)").textContent.trim()) : new Date(0);
      ...
      function renderTree(node, fragment, depth, filter) {
        ...
        for (const child of node.children) {
          ...
          if (child.type === "folder") {
            ...
            td.textContent = (expandedFolders.has(child.path) ? "▼ " : "► ") + child.name;
            tr.appendChild(td);
      ...
      function prepHtml() {
        trs.forEach((tr) => {
          ...
          if (center)
            center.outerHTML = center.innerHTML;
        });
      }
      prepHtml();
      loadState();
